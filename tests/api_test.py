...
def preprocess_image(image_path: str) -> Union[np.ndarray, None]:
    ...
    try:
        ...
        print(f"Applied preprocessing for EasyOCR: Grayscale, Contrast Adj, Thresholding to {image_path}")
...
def encode_image_to_base64(image_path: str) -> Union[str, None]:
    ...
    try:
        with open(image_path, "rb") as image_file:
            ...
            else: mime_type = "image/jpeg"; print(f"Warning: Unknown image type for {image_path}, assuming JPEG.")
...
def perform_ocr_openai(image_path: str, client) -> Union[str, None]:
    ...
    try:
        ...
        if response.choices and response.choices[0].message and response.choices[0].message.content:
            ...
            return extracted_text
        else: print("Error: OpenAI response did not contain expected text."); print("Response Choice:", response.choices[0] if response.choices else "No choices"); return None
    except openai.APIError as e: print(f"OpenAI API Error: {e}"); return None
    except openai.APIConnectionError as e: print(f"OpenAI Connection Error: {e}"); return None
    except openai.RateLimitError as e: print(f"OpenAI Rate Limit Error: {e}"); return None
    except openai.AuthenticationError as e: print(f"OpenAI Authentication Error: {e}"); return None
    except openai.InvalidRequestError as e: print(f"OpenAI Invalid Request Error: {e}"); return None
    except Exception as e: print(f"An unexpected error occurred during OpenAI OCR: {e}"); return None
...
def apply_fuzzy_matching(text: str, dictionary: list, threshold: int) -> Union[str, None]:
    ...
    for line in lines:
        ...
        corrected_words = []
        for word in words:
            if len(word) < 3 or word.isdigit(): corrected_words.append(word); continue
            best_match, score = process.extractOne(word, dictionary, scorer=fuzz.ratio)
            if score >= threshold and word != best_match: corrected_words.append(best_match); corrections_made += 1
...
def get_document_details(ocr_text: str, user_question: str, bluehive_key: str) -> Union[dict, None]:
    """
    ...
    try:
        response = requests.post(BLUEHIVE_API_ENDPOINT, headers=bh_headers, json=payload, timeout=90) # Increased timeout slightly
        response.raise_for_status()
        try:
            return response.json()
        except json.JSONDecodeError: print("Error parsing JSON response from BlueHive. Response text:", response.text); return {"error": "Failed to parse JSON response", "raw_response": response.text}
    except requests.exceptions.RequestException as e: print(f"BlueHive API Request failed: {e}"); return None
# === END Shared Logic ===
...
def main(ocr_method: str, image_path: str):
    ...
    if result:
        ...
        if isinstance(result, dict) and "error" in result:
             ...
             if "raw_response" in result: print(f"Raw response snippet: {result['raw_response'][:500]}...")
        else:
...
if __name__ == "__main__":
    # --- Argument Parsing ---
    parser = argparse.ArgumentParser(description="Perform OCR on an image using a selected method and analyze with BlueHive API.")
    ...
    try:
        ...
        import argparse
         ...
         print("Please install requirements: pip install requests python-levenshtein thefuzz python-dotenv")
         sys.exit(1)
    ...
    try:
        ...
        print("For 'gemini': pip install google-generativeai Pillow") # Pillow is needed for PIL.Image
        sys.exit(1)
